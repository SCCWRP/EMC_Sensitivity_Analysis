right_riemann <- function(sample_bin_breaks, flow, time_unit) {
  ...
  # then right riemann is matrix multiplication of time difference vector (scaled
  # to appropriate time_unit) and the second through n flow values (right values)
  sapply(df_list, function(x) {
      diff(x[, "mins"] * time_multiplier) %*% x[2:nrow(x), "flow_values"]
    }
  )
}
